...
class FileManage extends Model {
    ...
    public function __construct($path, $uid, $byId = false) {
        ...
        switch ($this->policyData["policy_type"]) {
            case 'local':
                $this->adapter = new LocalAdapter($this->fileData, $this->policyData, $this->userData);
                break;
//            case 'qiniu':
...
//                break;
     ...
     * @return void
     */
    public function Source($isapi = false) {
        if ($isapi == true) {
            if (!$this->policyData["origin_link"]) {
                return '此文件不支持获取源文件URL';
            } else {
                return $this->policyData["url"] . $this->fileData["pre_name"];
            }
        } else {
            if (!$this->policyData["origin_link"]) {
    ...
    static function RenameHandler($fname, $new, $uid, $notEcho = false) {
        ...
        if (empty($fileRecord)) {
           return self::folderRename($originFolder, $folderTmp, $uid, $notEcho);
            die();
        }
...
//                self::deleteFileRecord(array_column($value, 'id'), array_sum(array_column($value, 'size')), $value[0]["upload_user"]);
    ...
    static function MoveHandler($file, $dir, $new, $uid) {
        if (in_array($new, $dir)) {
            return '{ "result": { "success": false, "error": "不能移动目录到自身" } }';
        }
        ...
        if ($nameCheck || $dirCheck) {
          return '{ "result": { "success": false, "error": "文件名冲突，请检查是否重名" } }';
        }
        if (!empty($dir)) {
            return '{ "result": { "success": false, "error": "暂不支持移动目录" } }';
        }
        Db::name('files')->where([
            'upload_user' => $uid,
            'dir' => ["in", $movePath],
