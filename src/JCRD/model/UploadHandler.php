...
class UploadHandler extends Model
{
    ...
    public $callBackKey;
    //分片大小
    public $chunk_size = 10 * 1024 * 1024;
    ...
    public function combineChunks($fname)
    {
        ...
        foreach ($fname as $key => $value) {
            ...
            $content = fread($chunkObj, $this->chunk_size);
            fwrite($fileObj, $content, $this->chunk_size);
            unset($content);
            fclose($chunkObj);
    ...
    {
        header("HTTP/1.1 401 Unauthorized");
        file_put_contents('aa.txt', var_export($text, true), FILE_APPEND);
        if ($delete) {
